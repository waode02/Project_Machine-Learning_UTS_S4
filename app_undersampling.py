...
with tab1:
    ...
    if submitted:
        ...
        st.markdown("""
        <div class="warn-box">
        ⚠️ <strong>Disclaimer:</strong> Hasil prediksi ini bersifat informatif dan <strong>bukan diagnosa medis</strong>.
        Silakan konsultasikan kondisi kesehatan Anda dengan dokter atau tenaga medis profesional.
        </div>
        """, unsafe_allow_html=True)
...
with tab2:
    ...
    st.markdown("""
    Upload file CSV yang berisi data beberapa pasien sekaligus.
    CSV harus memiliki **18 kolom fitur** sesuai urutan di bawah.
    """)

    # Template Download
    template_data = {
        'taking_bp_meds': [1, 0],
        'taking_cholesterol_meds': [1, 0],
        'age': [65.0, 30.0],
        'sex': [1.0, 2.0],
        'race_ethnicity': [4.0, 6.0],
        'education': [2.0, 5.0],
        'poverty_income_ratio': [1.2, 4.5],
        'taking_insulin': [0, 0],
        'taking_diabetes_pills': [0, 0],
        'told_prediabetes': [1, 0],
        'told_stroke': [1, 0],
        'family_history_heart_attack': [1, 0],
        'diabetes': [0, 0],
        'smoking_ever': [1, 0],
        'smoking_current': [0, 0],
    ...
    uploaded_file = st.file_uploader("Upload CSV Pasien", type=["csv"])

    if uploaded_file:
        try:
            df_up = pd.read_csv(uploaded_file)
            st.write(f"✅ File berhasil dibaca: **{len(df_up)} pasien**")

            # Validasi kolom
            missing_cols = [c for c in FEAT_NAMES if c not in df_up.columns]
            if missing_cols:
                st.error(f"❌ Kolom berikut tidak ditemukan di CSV: `{missing_cols}`")
            else:
                df_input = df_up[FEAT_NAMES].copy()

                with st.spinner("Memproses prediksi..."):
                    results = []
                    for i, row in df_input.iterrows():
                        h = predict_single(row.to_dict())
                        results.append({
                            'No'                 : i + 1,
                            'Usia'               : row['age'],
                            'Jenis Kelamin'      : 'Laki-laki' if row['sex'] == 1.0 else 'Perempuan',
                            'Prediksi'           : '🔴 Berisiko' if h['prediksi'] == 1 else '🟢 Aman',
                            'Prob Sakit (%)'     : f"{h['prob_positif']:.1f}%",
                            'Prob Tidak Sakit (%)': f"{h['prob_negatif']:.1f}%",
                        })

                df_results = pd.DataFrame(results)

                # Ringkasan
                n_risiko = sum(1 for r in results if '🔴' in r['Prediksi'])
                n_aman   = len(results) - n_risiko

                col_r1, col_r2, col_r3 = st.columns(3)
                with col_r1:
                    st.metric("Total Pasien", len(results))
                with col_r2:
                    st.metric("🔴 Berisiko", n_risiko)
                with col_r3:
                    st.metric("🟢 Aman", n_aman)

                st.dataframe(df_results, use_container_width=True)

                # Download hasil
                csv_hasil = df_results.to_csv(index=False).encode('utf-8')
                st.download_button(
